...
func SingleAccGenSignedTxDataCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "gen-single-signed-tx",
		Short: "generate signed tx data",
		Long:  `batch generate signed tx data`,
		Example: `mock gen-single-signed-tx --num {num} --faucet-name {faucet-name}\&
--chain-id {chain-id} --node {node-url} --home {homeDir} --output {optional} --address {faucetAddress}`,
	...
	cmd.Flags().AddFlagSet(singleTxFlagSet)
...
func SingleSignByTendAndBroadcast() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "gen-single-signed-tx",
		Short: "generate signed tx data",
		Long:  `batch generate signed tx data`,
